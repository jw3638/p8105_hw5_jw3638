...
summary(homicide_df)
```
The raw dataset contains 52,179 rows, with each row representing a single homicide case reported in one of 50 large U.S. cities between 2007 and 2017. The dataset includes key variables such as case identifier, the date the homicide was reported, victim’s name, demographic characteristics of the victim, location of the homicide, geographic coordinates of the incident, and case outcome.
...
The warning from prop.test() (“Chi-squared approximation may be incorrect”) occurs in cities with very small sample sizes and does not affect the analysis.
...
```
The plot shows substantial variation in the proportion of homicides that remain unsolved across the 50 cities. Chicago, New Orleans, and Baltimore have the highest estimated proportions of unsolved cases (around 65–75%), and relatively narrow confidence intervals due to their large number of total homicides. In contrast, Tulsa (Alabama), Richmond, and Charlotte have much lower proportions (around 25–30%) and wider confidence intervals, reflecting smaller case counts and more uncertainty in the estimates.
